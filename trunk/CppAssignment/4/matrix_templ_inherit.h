...
// default constructor
template <typename T>
Matrix<T>::Matrix() : Vector(), nrows(0), ncols(0)
{
}

// alternate constructor 
template <typename T>
Matrix<T>::Matrix(int rows, int cols) : Vector(rows * cols) , nrows(rows), ncols(cols)
{
}
...
T& Matrix<T>::operator()(int i, int j)
{
	...
	return this->operator() operator[i * ncols + j];
}
